...
@objc class AppDelegate: FlutterAppDelegate, FlutterStreamHandler {
  ...
  public func onListen(withArguments arguments: Any?,
    ...
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(AppDelegate.hideScreen),
      name: UIScreen.capturedDidChangeNotification,
      object: nil)

     NotificationCenter.default.addObserver(
       self,
       selector: #selector(AppDelegate.userDidScreenshot),
    ...
    @objc private func hideScreen() {
        print("User Started Recording The Screen \n Make The Screen black")
        if UIScreen.main.isCaptured {
            window?.isHidden = true
        } else {
        print("User Stoped Recording The creen \n Return the Screen UI")
            window?.isHidden = false
        }
    }
    
  @objc private func userDidScreenshot() {
        print("Screen capture detected \n force the immediate exit of the app!")
        exit(0)
     }
